...
void Lowering::RehomeArgForFastTailCall(
    unsigned int lclNum, GenTree* insertTempBefore, GenTree* overwritingNode, GenTreeCall* callNode)
{
    ...
    for (GenTree* treeNode = overwritingNode; treeNode != callNode; treeNode = treeNode->gtNext)
    {
        ...
        if (tmpLclNum == BAD_VAR_NUM)
        {
            ...
            if (tmpTyp == TYP_STRUCT)
            {
                ...
                GenTreeBlk* storeBlk =
                    new (comp, GT_STORE_BLK) GenTreeBlk(GT_STORE_BLK, TYP_STRUCT, loc, value, callerArgDsc->lvExactSize);
